...
var ThemeProvider = ({ children, config }) => {
  ...
  const [themeState, internalSetThemeState] = useState(() => {
  ...
  const contextValue = useMemo(() => __spreadProps(__spreadValues({}, themeState), { setTheme }), [themeState, setTheme]);
  return /* @__PURE__ */ jsx(ThemeContext.Provider, { value: contextValue, children });
};
var ThemeScript = memo(({ config }) => {
  const resolvedConfig = useMemo(() => {
    return createThemeConfig(config);
  }, [config]);
  const cssVarNames = {
